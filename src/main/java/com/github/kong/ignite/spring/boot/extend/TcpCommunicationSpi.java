...
import static com.github.kong.ignite.spring.boot.extend.TcpCommunicationConnectionCheckFuture.SES_FUT_META;
...
import static org.apache.ignite.spi.communication.tcp.messages.RecoveryLastReceivedMessage.*;
 ...
 *             &lt;bean class="TcpCommunicationSpi"&gt;
 *                 &lt;!-- Override local port. --&gt;
...
public class TcpCommunicationSpi extends IgniteSpiAdapter implements CommunicationSpi<Message> {
    ...
    /** */
    private TcpCommunicationSpi.ConnectGateway connectGate;

    /** */
    private TcpCommunicationSpi.ConnectionPolicy connPlc = new TcpCommunicationSpi.FirstConnectionPolicy();

    /** Channel connection index provider. */
    private TcpCommunicationSpi.ConnectionPolicy chConnPlc;
    ...
    private final GridNioServerListener<Message> srvLsnr =
            new GridNioServerListenerAdapter<Message>() {
                ...
                @Override public void onConnected(GridNioSession ses) {
                    //在节点中断后，节点重新连接的时候，都重现建立连接，覆盖原来的
                    if (ses.accepted()) {
...
//                                    ", rmtAddr=" + ses.remoteAddress() + ']');
                            ...
                            if (outDesc != null) {
                                if (outDesc.nodeAlive(getSpiContext().node(id))) {
                                    if (!outDesc.messagesRequests().isEmpty()) {
                                        ...
                                        TcpCommunicationSpi.DisconnectedSessionInfo disconnectData =
                                                new TcpCommunicationSpi.DisconnectedSessionInfo(outDesc, connId.connectionIndex());
                ...
                private void onFirstMessage(final GridNioSession ses, Message msg) {
                    ...
                    if (isChannelConnIdx(msg0.connectionIndex()))
                        ...
                        if (oldFut == null) {
                            ...
                            if (oldFut instanceof TcpCommunicationSpi.ConnectFuture && locNode.order() < rmtNode.order()) {
                                if (log.isInfoEnabled()) {
    ...
    private TcpCommunicationSpi.CommunicationWorker commWorker;

    /** Shared memory accept worker. */
    private TcpCommunicationSpi.ShmemAcceptWorker shmemAcceptWorker;

    /** Shared memory workers. */
    private final Collection<TcpCommunicationSpi.ShmemWorker> shmemWorkers = new ConcurrentLinkedDeque<>();
    ...
    /** */
    private final GridLocalEventListener discoLsnr = new TcpCommunicationSpi.DiscoveryListener();
    ...
    @Override public Map<String, Object> getNodeAttributes() throws IgniteSpiException {
        ...
        if (connectionsPerNode > 1)
            connPlc = new TcpCommunicationSpi.RoundRobinConnectionPolicy();
        else
            connPlc = new TcpCommunicationSpi.FirstConnectionPolicy();

        chConnPlc = new TcpCommunicationSpi.ConnectionPolicy() {
    ...
    @Override public void spiStart(String igniteInstanceName) throws IgniteSpiException {
        ...
        registerMBean(igniteInstanceName, new TcpCommunicationSpi.TcpCommunicationSpiMBeanImpl(this), TcpCommunicationSpiMBean.class);

        connectGate = new TcpCommunicationSpi.ConnectGateway();

        if (shmemSrv != null) {
            shmemAcceptWorker = new TcpCommunicationSpi.ShmemAcceptWorker(shmemSrv);
        ...
        commWorker = new TcpCommunicationSpi.CommunicationWorker(igniteInstanceName, log);
    ...
    private GridCommunicationClient reserveClient(ClusterNode node, int connIdx) throws IgniteCheckedException {
        ...
        while (true) {
            ...
            if (client == null) {
                ...
                GridFutureAdapter<GridCommunicationClient> fut = new TcpCommunicationSpi.ConnectFuture();
     ...
     * </p>
     * <p>
     *     <em>Note.</em> The {@link TcpCommunicationSpi.HandshakeTimeoutObject} is created to control execution timeout during the
     *     whole handshaking process.
     * </p>
     *
     * @param node Remote node identifier to connect with.
     * @param connIdx Connection index based on configured {@link TcpCommunicationSpi.ConnectionPolicy}.
    ...
    private GridNioSession createNioSession(ClusterNode node, int connIdx) throws IgniteCheckedException {
        ...
        for (InetSocketAddress addr : addrs) {
            ...
            while (ses == null) { // Reconnection on handshake timeout.
                ...
                TcpCommunicationSpi.CommunicationWorker commWorker0 = commWorker;
    ...
    private void safeShmemHandshake(
        ...
        TcpCommunicationSpi.HandshakeTimeoutObject<GridCommunicationClient> obj = new TcpCommunicationSpi.HandshakeTimeoutObject<>(client,
                U.currentTimeMillis() + timeout);

        addTimeoutObject(obj);

        try {
            client.doHandshake(new TcpCommunicationSpi.HandshakeClosure(rmtNodeId));
        }
        finally {
            if (obj.cancel())
    ...
    private long safeTcpHandshake(
        ...
        TcpCommunicationSpi.HandshakeTimeoutObject obj = new TcpCommunicationSpi.HandshakeTimeoutObject<>(ch, U.currentTimeMillis() + timeout);
        ...
        @Override protected void body() throws InterruptedException {
            try {
                while (!Thread.interrupted()) {
                    TcpCommunicationSpi.ShmemWorker e = new TcpCommunicationSpi.ShmemWorker(srv.accept());
    ...
    private class ShmemWorker extends GridWorker {
        ...
        /** {@inheritDoc} */
        @Override public String toString() {
            return S.toString(TcpCommunicationSpi.ShmemWorker.class, this);
        }
    }
    ...
    private class CommunicationWorker extends GridWorker {
        /** */
        private final BlockingQueue<TcpCommunicationSpi.DisconnectedSessionInfo> q = new LinkedBlockingQueue<>();
         ...
         */
        private void processDisconnect(TcpCommunicationSpi.DisconnectedSessionInfo sesInfo) {
         ...
         * @param sesInfo Disconnected session information.
         */
        void addProcessDisconnectRequest(TcpCommunicationSpi.DisconnectedSessionInfo sesInfo) {
    ...
    private static class HandshakeTimeoutObject<T> implements IgniteSpiTimeoutObject {
        ...
        /** {@inheritDoc} */
        @Override public String toString() {
            return S.toString(TcpCommunicationSpi.HandshakeTimeoutObject.class, this);
        }
    }
    ...
    private static class DisconnectedSessionInfo {
        ...
        /** {@inheritDoc} */
        @Override public String toString() {
            return S.toString(TcpCommunicationSpi.DisconnectedSessionInfo.class, this);
        }
    }
    ...
    /** */
    private static class FirstConnectionPolicy implements TcpCommunicationSpi.ConnectionPolicy {
        /** {@inheritDoc} */
        @Override public int connectionIndex() {
            return 0;
        }
    ...
    /** */
    private class RoundRobinConnectionPolicy implements TcpCommunicationSpi.ConnectionPolicy {
        /** {@inheritDoc} */
        @Override public int connectionIndex() {
